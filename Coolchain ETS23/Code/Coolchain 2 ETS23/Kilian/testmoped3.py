...
def read_transid():
    ...
    text_direction_errors.delete("1.0", tk.END)
...
def verifikation_auswertung(transid):
    ...
    # Direction Fehler prüfen
    direction_errors = []
    
    if daten_id[0]["direction"] != "'in'":
        direction_errors.append(f"Fehler: Erster Eintrag ist nicht 'in'.")

    for i in range(1, len(daten_id)):
        if daten_id[i]["direction"] == daten_id[i-1]["direction"]:
            direction_errors.append(f"Fehler: Falsche Reihenfolge an Position {i+1} ({daten_id[i]['direction']} nach {daten_id[i-1]['direction']}).")

    if daten_id[-1]["direction"] != "'out'":
        direction_errors.append(f"Fehler: Letzter Eintrag ist nicht 'out'.")

    text_direction_errors.delete("1.0", tk.END)
    if direction_errors:
        text_direction_errors.insert(tk.END, "\n".join(direction_errors))
        text_direction_errors.config(fg="red")
    else:
        text_direction_errors.insert(tk.END, "Directionlogik OK.")
        text_direction_errors.config(fg="green")

def start_fenster_manuell():
    global combobox_transid, label_duration, text_temperature, text_direction_errors, tree, fenster_manuell

    fenster_manuell = tk.Toplevel(fenster_hauptmenue)
    fenster_manuell.title("Manuelle Überprüfung")
    fenster_manuell.geometry("1000x700")
    ...
    text_direction_errors = scrolledtext.ScrolledText(fenster_manuell, width=80, height=5, wrap=tk.WORD, font=("Helvetica", 12))
    text_direction_errors.grid(column=0, row=5, columnspan=3, padx=10, pady=10)
