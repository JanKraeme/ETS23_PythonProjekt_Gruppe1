...
def datenauswertung_csv():
    ...
    for transid_csv in transid_val:
        ...
        tree.insert("", "end", values=(transid_nr ,transid_csv , "verifikation_string"))
...
def verifikation_auswertung(transid):
    global daten_id, verifikation_string
    for item in tree.get_children():
        tree.delete(item)
...
def check_direction(daten_direction):
    ...
    if last_direction == "'out'":
        ...
        verifikation_string = 'Auschecken am Ende fehlt'
        label_direction.config(text=verifikation_string, fg="red")
        return False
...
#Button Listen überprüfen
button2 = tk.Button(fenster_hauptmenue, text="Transport-ID Listen prüfen", command=button_click_2, bg="#007BFF", fg="white", font=("Helvetica", 12))
