...
def start_fenster_manuell():
    def zeiten_auswertung(transid):
        ...
        for i, row in enumerate(daten):
            ...
            direction = row[2].lower()
            timestamp = row[3]
            
            if direction == 'out':
                last_out = (row[1], timestamp)
            elif direction == 'in' and last_out:
                out_station_id, out_time = last_out
                time_diff = (timestamp - out_time).total_seconds()
                
                if time_diff > 600:
                    messagebox.showwarning("Warnung", f"Übergabe > 10min ({int(time_diff)} Sek.). Wetter: {temp}")
                last_out = None
    ...
    label_duration = tk.Label(fenster)
    label_duration.pack()
