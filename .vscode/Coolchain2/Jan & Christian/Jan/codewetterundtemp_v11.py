...
def temperatur_überwachung():
    ...
    tree_temp = ttk.Treeview(fenster_temp, columns=("stationID", "datetime", "temp"), show='headings')
    for col in ["stationID", "datetime", "temp"]:
        tree_temp.heading(col, text=col.capitalize())
    tree_temp.pack(expand=True, fill='both')

    for eintrag in daten_Temp:
        if eintrag['temp'] < 2 or eintrag['temp'] > 4:
            tree_temp.insert("", "end", values=(eintrag['stationID'], eintrag['datetime'], f"{eintrag['temp']} °C"))
...
def lade_db_daten():
    global db_daten, db_datetime, db_direction, db_zwischenzeit, transportstation_plz
    conn = connect_db()
    if not conn:
        return
    cursor = conn.cursor()
    ...
    db_direction = []
    db_zwischenzeit = []
    transportstation_plz = {}

    transport_ids = set()

    # Coolchain (unverschlüsselt)
    ...
    for row in cursor.fetchall():
        ...
        decrypted_transportstationid = decrypt_value(row[2])
        db_daten.append({'companyid': decrypted_companyid, 'transportid': row[1],
                         'transportstationid': decrypted_transportstationid, 'direction': row[3], 'datetime': row[4]})
        transport_ids.add(row[1])

    # Datetime
    cursor.execute('SELECT transportid, datetime, direction FROM coolchain')
    for row in cursor.fetchall():
        db_datetime.append({'transportid': row[0], 'datetime': row[1], 'direction': row[2]})

    # Direction
    cursor.execute('SELECT transportid, direction FROM coolchain')
    for row in cursor.fetchall():
        db_direction.append({'transportid': row[0], 'direction': row[1]})

    # Zwischenzeit
    cursor.execute('SELECT transportid, transportstationid, direction FROM coolchain')
    for row in cursor.fetchall():
        db_zwischenzeit.append({'transportid': row[0], 'transportstationid': row[1], 'direction': row[2]})

    # Transportstation PLZ
    cursor.execute('SELECT transportstationID, plz FROM transportstation')
    for row in cursor.fetchall():
        transportstation_plz[row[0]] = row[1]
...
def zeiten_auswertung(transid):
    ...
    for eintrag in daten_id:
        plz = transportstation_plz.get(eintrag["transportstationid"], "0")
        wetter = wetter_abfrage(plz, eintrag["datetime"])
        tree.insert("", "end", values=(eintrag["companyid"], eintrag["transportstationid"], eintrag["direction"], eintrag["datetime"], wetter))
    ...
    for i in range(1, len(daten_datetime_values) - 1, 2):
        ...
        if out_record['direction'].strip("'") == 'out' and in_record['direction'].strip("'") == 'in':
            ...
            if time_diff > 600:
                ...
                uebergabe_korrekt = False
                break
    if uebergabe_korrekt and in_out_korrekt:
        label_direction.config(text="In/Out Reihenfolge und Übergabezeiten korrekt.", fg="green")

    # -------------------- Letzte Station vs. Aktion prüfen (gleiche Station mehrmals?) --------------------
    letzte_station = {}
    letzte_aktion = {}
    station_korrekt = True
    ...
    for eintrag in daten_zwischenzeit:
        ...
        if transportid in letzte_station:
            if letzte_aktion[transportid] == 'out' and aktion == 'in' and letzte_station[transportid] == station:
                ...
                station_korrekt = False
                break

        letzte_station[transportid] = station
        letzte_aktion[transportid] = aktion

    if uebergabe_korrekt and in_out_korrekt and station_korrekt:
...
def start_fenster_manuell():
    ...
    tree = ttk.Treeview(fenster, columns=("companyid", "station", "direction", "datetime", "Wetter"), show='headings')
    for col in ["companyid", "station", "direction", "datetime", "Wetter"]:
